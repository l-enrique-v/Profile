...
{
    public class FileService : IFileService, IMapFile
    {
        ...
        public async Task<string> UploadFile(IFormFile files, string fileName)
        {
            ...
            //string inputString = string.Format("userfile_{0}_{1}", System.IO.Path.GetRandomFileName(), "_-_",files.FileName);
            await fileTransferUtility.UploadAsync(files.OpenReadStream(), bucketName, fileName);

            return string.Format("https://{0}.s3.{1}.amazonaws.com/{2}", bucketName, bucketRegion, fileName);
        }
